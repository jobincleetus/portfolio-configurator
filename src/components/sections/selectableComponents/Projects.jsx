...
const Projects = ({handleCancel, handleDelete, ...props}) => {
  ...
  useEffect(() => {
    if(currentCard.title && currentCard.description && currentCard.link) {
  ...
  useEffect(() => {
    const existingData = content.find((r) => r.name === 'Projects')
  ...
  return (
    ...
    {
      showTools &&
      <ToolBar setIsEditing={setIsEditing} contentName={'Projects'} handleDelete={handleDelete} />
    }
    {
    isEditing && 
        <EditOptions isDisabled={isDisabled} contentName={'Projects'} fieldData={data} setIsEditing={setIsEditing} handleCancel={handleCancel} />
    }
            ...
            <div className='grid lg:grid-cols-2 gap-8'>
                {
                    data.list && data.list.length > 0 && data.list.map((card, i) => (
                        <div key={`card_${i}`} className='relative rounded-2xl px-8 py-6 flex flex-col justify-between bg-white border'>
                            ...
                            <div>
                                <img src={card.image ? card.image : imgPlaceHolder} className={'w-10 h-10 mb-3 rounded-md object-cover overflow-hidden'} />
                                <p className='w-full mb-2 text-black'>{card.title}</p>
                                <div className='flex gap-2 items-center mb-3'>
                                    <FaLink className='text-secondary text-secondary' />
                                    <p className='w-full text-sm'>{card.link}</p>
                                </div>
                                <p className='w-full text-sm text-black'>{card.description}</p>
                            </div>
                        </div>
                    ))
                }
                {
                    ...
                    <div className='relative rounded-2xl px-8 py-6 flex flex-col justify-between bg-white border'>
                        <div>
                            ...
                            />
                            <input type="text" placeholder='Enter project title' className='w-full mb-2 placeholder-black' onChange={(text) => handleCurrentCardChange('title', text.target.value)} />
                            <div className='flex gap-2 items-center mb-3'>
